...
extension DateSection {
    ...
    var backgroundRectangle: some View {
        RoundedRectangle(cornerRadius: 18)
            ...
            .background { Color.makeAiachyColor(aiachyState, aiachyColor: selectedCurrentTime == currentTime ?  .fifthColor : .backgroundAlternativeColor)
                    .cornerRadius(18) 
                .shadow(color: Color.makeAiachyColor(aiachyState, aiachyColor: .firstColor).opacity(0.5),
                        radius: 4,
                        x: 0,
                        y: 5)}
            .frame(height: ACYdh(aiachyState, d: 0.05))
